...
def hand_capture(frame_in, box_x, box_y):
    ...
    for i in list(range(capture_box_count)):
        ROI[i * capture_box_dim:i * capture_box_dim + capture_box_dim, 0:capture_box_dim] = hsv[box_y[i]:box_y[
                                                                                                             i] + capture_box_dim,
                                                                                            box_x[i]:box_x[
                                                                                                         i] + capture_box_dim]
...
def mark_hand_center(frame_in, cont):
    ...
    for ind_y in list(range(int(y + 0.3 * h),int(y + 0.8 * h))):  # around 0.25 to 0.6 region of height (Faster calculation with ok results)
        for ind_x in list(range(int(x + 0.3 * w),int(x + 0.6 * w))):  # around 0.3 to 0.6 region of width (Faster calculation with ok results)
